...
func resourceLogSyslogd3OverrideFilterUpdate(d *schema.ResourceData, m interface{}) error {
	...
	obj, err := getObjectLogSyslogd3OverrideFilter(d, c.Fv)
	if err != nil {
		return fmt.Errorf("error updating LogSyslogd3OverrideFilter resource while getting object: %v", err)
	}

	o, err := c.UpdateLogSyslogd3OverrideFilter(obj, mkey, vdomparam, urlparams, batchid)
	if err != nil {
...
func resourceLogSyslogd3OverrideFilterDelete(d *schema.ResourceData, m interface{}) error {
	...
	err := c.DeleteLogSyslogd3OverrideFilter(mkey, vdomparam, batchid)
	if err != nil {
...
func resourceLogSyslogd3OverrideFilterRead(d *schema.ResourceData, m interface{}) error {
	...
	o, err := c.ReadLogSyslogd3OverrideFilter(mkey, vdomparam, urlparams, batchid)
	if err != nil {
...
func flattenLogSyslogd3OverrideFilterFreeStyle(v interface{}, d *schema.ResourceData, pre string, sv string) []map[string]interface{} {
	...
	for _, r := range l {
		...
		if _, ok := i["id"]; ok {
			...
			tmp["id"] = flattenLogSyslogd3OverrideFilterFreeStyleId(i["id"], d, pre_append, sv)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "category"
		if _, ok := i["category"]; ok {

			tmp["category"] = flattenLogSyslogd3OverrideFilterFreeStyleCategory(i["category"], d, pre_append, sv)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "filter"
		if _, ok := i["filter"]; ok {

			tmp["filter"] = flattenLogSyslogd3OverrideFilterFreeStyleFilter(i["filter"], d, pre_append, sv)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "filter_type"
		if _, ok := i["filter-type"]; ok {

			tmp["filter_type"] = flattenLogSyslogd3OverrideFilterFreeStyleFilterType(i["filter-type"], d, pre_append, sv)
		}
...
func getObjectLogSyslogd3OverrideFilter(d *schema.ResourceData, sv string) (*map[string]interface{}, error) {
