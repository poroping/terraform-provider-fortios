...
func resourceLogSyslogd4FilterUpdate(d *schema.ResourceData, m interface{}) error {
	...
	obj, err := getObjectLogSyslogd4Filter(d, c.Fv)
	if err != nil {
		return fmt.Errorf("error updating LogSyslogd4Filter resource while getting object: %v", err)
	}

	o, err := c.UpdateLogSyslogd4Filter(obj, mkey, vdomparam, urlparams, batchid)
	if err != nil {
...
func resourceLogSyslogd4FilterDelete(d *schema.ResourceData, m interface{}) error {
	...
	err := c.DeleteLogSyslogd4Filter(mkey, vdomparam, batchid)
	if err != nil {
...
func resourceLogSyslogd4FilterRead(d *schema.ResourceData, m interface{}) error {
	...
	o, err := c.ReadLogSyslogd4Filter(mkey, vdomparam, urlparams, batchid)
	if err != nil {
...
func flattenLogSyslogd4FilterFreeStyle(v interface{}, d *schema.ResourceData, pre string, sv string) []map[string]interface{} {
	...
	for _, r := range l {
		...
		if _, ok := i["id"]; ok {
			...
			tmp["id"] = flattenLogSyslogd4FilterFreeStyleId(i["id"], d, pre_append, sv)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "category"
		if _, ok := i["category"]; ok {

			tmp["category"] = flattenLogSyslogd4FilterFreeStyleCategory(i["category"], d, pre_append, sv)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "filter"
		if _, ok := i["filter"]; ok {

			tmp["filter"] = flattenLogSyslogd4FilterFreeStyleFilter(i["filter"], d, pre_append, sv)
		}

		pre_append = pre + "." + strconv.Itoa(con) + "." + "filter_type"
		if _, ok := i["filter-type"]; ok {

			tmp["filter_type"] = flattenLogSyslogd4FilterFreeStyleFilterType(i["filter-type"], d, pre_append, sv)
		}
...
func getObjectLogSyslogd4Filter(d *schema.ResourceData, sv string) (*map[string]interface{}, error) {
